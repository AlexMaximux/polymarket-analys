 ...
 * only way to backtest signals at executable prices instead of midpoints.
 */
...
export async function recordUpdownTicks(coins = RECORDER_COINS): Promise<number> {
  const snaps = await Promise.all(coins.map(c => fetchUpdownSnapshot(c)));
  const lines: string[] = [];
  for (const d of snaps) {
    if (!d || !d.m1h) continue;
    lines.push(JSON.stringify({
      ts: d.serverTime, coin: d.coin, t: r(d.t, 3), venue: d.priceVenue ?? null,
